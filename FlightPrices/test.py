...
class MakeMyTrip(object):
    ...
    def journey_roundtrip(self, origin, destination, depart_date, return_date, adult=1, children=0, infant=0):
        new_url = BASE_URL + 'splitRTDataService.json?classType=E&deptDate=' + depart_date + '&fltMap=&fromCity=' + origin + '&noOfAdlts=' + str(
            adult) + \
                  '&noOfChd=' + str(children) + '&noOfInfnt=' + str(
            infant) + '&returnDate=' + return_date + '&toCity=' + destination + '&tripType=R&tripTypeDup=R'
    ...
    def print_json(self, l):
        ...
        for i in range(tmp_size):
            ...
            ""
            print
            u"\033[1m" + l[i]['le'][0]['an'], u"\033[0m      \u20B9 \033[92m", l[i]['af'], "\033[0m  in  ", l[i]['td']
            layover = self.get_extra_detail(l[i]['le'])
            # ToDo
            print
            l[i]['le'][0]['fd'], l[i]['le'][0]['fdt'], \
            u"  --->>  ", self.arrival_time
            print
            "\tStoppage : ", layover
